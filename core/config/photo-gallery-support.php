...
final class TuruncuWePhotoGallerySupportConfig
{
    private $post_type = "portfoy";

    function __construct()
    {
        add_action('admin_init', array($this, 'property_gallery_add_metabox'));
        add_action('admin_head-post.php', array($this, 'property_gallery_styles_scripts'));
        add_action('admin_head-post-new.php', array($this, 'property_gallery_styles_scripts'));
        add_action('save_post', array($this, 'property_gallery_save'));

    }
    public function property_gallery_add_metabox()
    {
        add_meta_box(
            'post_custom_gallery',
            'Gallery',
            array($this, 'property_gallery_metabox_callback'),
            $this->post_type,
            'normal',
            'core'
        );
    ...
    public function property_gallery_metabox_callback()
    {
        wp_nonce_field(basename(__FILE__), 'photo_gallery_support_nonce');
        global $post;
                    ...
                    for ($i = 0; $i < count($gallery_data['image_url']); $i++) {
                        ...
                        <div class="gallery_single_row dolu">
                            <div class="gallery_area image_container ">
                                <img class="gallery_img_img" src="<?php esc_html_e($gallery_data['image_url'][$i]); ?>" height="55"
    ...
    public function property_gallery_styles_scripts()
    {
        global $post;
    ...
    public function property_gallery_save($post_id)
    {
        ...
        $is_valid_nonce = (isset($_POST['photo_gallery_support_nonce']) && wp_verify_nonce($_POST['photo_gallery_support_nonce'], basename(__FILE__))) ? 'true' : 'false';
...
new TuruncuWePhotoGallerySupportConfig();
